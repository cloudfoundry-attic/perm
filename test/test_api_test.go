					...
					w.Write([]byte(`
{
	"keys": [
		...
		testCoreFunctionality(func() []api.ServerOption {
			return serverOpts
		}, func() []perm.DialOption {
			validToken, err := getSignedToken(validPrivateKey, validIssuer, validExpiryDate)
			Expect(err).ToNot(HaveOccurred())

			return []perm.DialOption{perm.WithTokenSource(&testTokenSource{token: validToken})}
		})
...
func testCoreFunctionality(serverOptsFactory func() []api.ServerOption, clientOptsFactory func() []perm.DialOption) {
	var (
		serverOpts []api.ServerOption
